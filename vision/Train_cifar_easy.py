...
parser = argparse.ArgumentParser(description='PyTorch CIFAR-Easy with DivideMix')
...
def save_checkpoint(args, epoch, state, file_dir):    
    directory = f"{file_dir}/cifar_easy/{args.noise_mode}_{args.r}/{args.filename}/"
...
def setup_logs(args, mode):
    file_dir = "results"
    if not args.no_log:
        files_dir = '%s/cifar_easy/%s_%s' %(file_dir, args.noise_mode, args.r)
...
def warmup(epoch,net,optimizer,dataloader):
    ...
    for batch_idx, (inputs, labels, path) in enumerate(dataloader):      
        ...
        if args.noise_mode in ['asym', 'adv']:  # penalize confident prediction for asymmetric noise
...
stats_log=open('./results/cifar_easy_%.2f_%s_clean%.2f_%s'%(args.r,args.noise_mode,args.clean_ratio, args.model)+'_stats.txt','w') 
test_log=open('./results/cifar_easy_%.2f_%s_clean%.2f_%s'%(args.r,args.noise_mode,args.clean_ratio,args.model)+'_acc.txt','w')     
...
loader = dataloader.cifar_dataloader(args.dataset, r=args.r, clean_ratio=args.clean_ratio, noise_mode=args.noise_mode, batch_size=args.batch_size, num_workers=5, root_dir="cifar_easy", log=stats_log, flip_type=flip_type, noise_file=f'./noise_jason/cifar_easy_{args.dataset}_{args.noise_mode}_{args.r}_clean_{args.clean_ratio}.json')
...
if args.noise_mode in ['asym', 'adv']:
    conf_penalty = NegEntropy()

all_loss = [[],[]] # save the history of losses from two networks

args.filename = f"{args.model}_lr{args.lr}_lambda{args.lambda_u}_clean{args.clean_ratio}_epochs{args.num_epochs}_seed{args.seed}_fliptype{flip_type}.log"
start_epoch = 0
mode = 'w+'
if args.checkpoint:
    checkpth = f"{args.checkpoint_dir}/cifar_easy/{args.noise_mode}_{args.r}/{args.filename}/checkpoint.pth.tar"
...
for epoch in range(start_epoch, args.num_epochs+1):                 
    ...
    for param_group in optimizer2.param_groups:
        param_group['lr'] = lr          
    test_loader = loader.run('binary_test')
    eval_loader = loader.run('binary')
    clean_loader = loader.run('binary_clean')

    if epoch<warm_up:       
        warmup_trainloader = loader.run('binary')
        print('Warmup Net1')
        ...
        stats_log.flush() 
        if sum(pred2) > 0:
            labeled_trainloader, unlabeled_trainloader = loader.run('binary_train',pred2,prob2) # co-divide    
        ...
        stats_log.flush() 
        if sum(pred1) > 0:
            labeled_trainloader, unlabeled_trainloader = loader.run('binary_train',pred1,prob1) # co-divide
    ...
    if args.save_model and epoch > args.num_epochs - 5: 
