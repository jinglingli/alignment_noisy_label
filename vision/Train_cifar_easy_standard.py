...
parser = argparse.ArgumentParser(description='PyTorch CIFAR-Easy with vanilla training')
parser.add_argument('--model', type=str, default='ResNet18', help='choose which model')
parser.add_argument('--batch_size', default=64, type=int, help='train batchsize') 
parser.add_argument('--lr', '--learning_rate', default=0.01, type=float, help='initial learning rate')
parser.add_argument('--noise_mode',  default='sym', choices=['sym', 'asym', 'adv'])
...
parser.add_argument('--lr_type', type=str, default='type1', help='lr scheduler type')
parser.add_argument('--ind', default=2, type=int)
...
parser.add_argument('--log_interval', type=int, default=300, help='how many epochs to save model')
...
def save_checkpoint(args, epoch, state, file_dir):    
    directory = f"./standard/{file_dir}/cifar_easy/{args.noise_mode}_{args.r}/{args.filename}/"
...
lr_schedules = {"type1": adjust_learning_rate1, "type2": adjust_learning_rate2}
adjust_learning_rate = lr_schedules[args.lr_type]
...
stats_log=open('./standard/results/%s_easy_%.2f_%s_clean%.2f_%s'%(args.dataset,args.r,args.noise_mode,args.clean_ratio, args.model)+'_stats.txt','w') 

loader = dataloader.cifar_dataloader(args.dataset, r=args.r, clean_ratio=args.clean_ratio, noise_mode=args.noise_mode, batch_size=args.batch_size, num_workers=5, root_dir="cifar_easy", log=stats_log, noise_file=f'./noise_jason/cifar_easy_{args.dataset}_{args.noise_mode}_{args.r}_clean_{args.clean_ratio}_{args.seed}.json')
...
args.filename = f"{args.model}_loss{loss_fn}_lr{args.lr}_{args.lr_type}_clean{args.train_only_clean}_{args.clean_ratio}_epochs{args.num_epochs}_seed{args.seed}.log"
start_epoch = 0
mode = 'w+'
if args.checkpoint:
    checkpth = f"./standard/{args.checkpoint_dir}/cifar_easy/{args.noise_mode}_{args.r}/{args.filename}/checkpoint.pth.tar"
...
if args.train_only_clean:
    trainloader = loader.run('binary_clean')   
else:
    trainloader = loader.run('binary')
